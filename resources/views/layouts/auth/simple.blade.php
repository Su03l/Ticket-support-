...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
  ...
  <body class="min-h-screen bg-zinc-950 text-zinc-950 antialiased dark:bg-zinc-950">
    ...
    <form method="POST" action="{{ route('language.switch', $alternateLocale) }}" class="fixed right-6 top-6 z-20 rtl:left-6 rtl:right-auto">
      @csrf
      <button type="submit" class="rounded-lg border border-white/15 bg-white/90 px-4 py-2 text-sm font-semibold text-zinc-900 shadow-sm backdrop-blur-md transition-all hover:bg-white dark:border-zinc-700 dark:bg-zinc-900 dark:text-white dark:hover:bg-zinc-800">
    ...
    <div class="grid min-h-svh lg:grid-cols-[minmax(0,1fr)_minmax(32rem,40rem)]">
      ...
      <section class="relative hidden overflow-hidden bg-zinc-950 text-white lg:flex">
        ...
        <div class="relative flex w-full flex-col justify-between p-12 xl:p-20">
          {{-- Logo --}}
          <a href="{{ route('home') }}" class="flex items-center gap-4 font-semibold text-xl" wire:navigate>
            <span class="flex size-10 items-center justify-center rounded-xl bg-white/10 ring-1 ring-white/20 backdrop-blur-md shadow-lg">
          ...
          <div class="max-w-xl">
            ...
            <h1 class="text-4xl font-semibold leading-snug xl:text-5xl">
          ...
          <div class="grid max-w-lg grid-cols-3 gap-4">
            <div class="rounded-xl border border-white/10 bg-white/5 p-5 backdrop-blur-md transition-transform hover:scale-105">
              <p class="text-2xl font-semibold">24/7</p>
              <p class="mt-1.5 text-xs font-medium text-zinc-400 uppercase">{{ __('Realtime ready') }}</p>
            </div>
            <div class="rounded-xl border border-white/10 bg-white/5 p-5 backdrop-blur-md transition-transform hover:scale-105">
              <p class="text-2xl font-semibold">{{ __('SLA') }}</p>
              <p class="mt-1.5 text-xs font-medium text-zinc-400 uppercase">{{ __('Breach tracking') }}</p>
            </div>
            <div class="rounded-xl border border-white/10 bg-white/5 p-5 backdrop-blur-md transition-transform hover:scale-105">
              <p class="text-2xl font-semibold">{{ __('Roles') }}</p>
              <p class="mt-1.5 text-xs font-medium text-zinc-400 uppercase">{{ __('Scoped access') }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="flex min-h-svh flex-col items-center justify-center bg-zinc-50 p-6 dark:bg-zinc-950 sm:p-12">
        <div class="w-full max-w-[400px]">
          {{-- Mobile logo --}}
          <a href="{{ route('home') }}" class="mb-10 flex items-center justify-center gap-3 font-semibold text-xl text-zinc-900 dark:text-white lg:hidden" wire:navigate>
          ...
          {{-- Auth form card --}}
          <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-lg shadow-zinc-200/30 dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none sm:p-10">
            {{ $slot }}
          </div>
