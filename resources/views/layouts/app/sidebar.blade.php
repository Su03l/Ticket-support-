...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}" class="{{ $themePreference === 'dark' ? 'dark' : '' }}">
    ...
    <body class="min-h-screen bg-zinc-50 text-zinc-900 antialiased dark:bg-zinc-950 dark:text-zinc-100 selection:bg-zinc-900 selection:text-white dark:selection:bg-white dark:selection:text-zinc-900">
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200/80 bg-white dark:border-zinc-800/80 dark:bg-zinc-900">
            <flux:sidebar.header class="border-b border-zinc-200/80 dark:border-zinc-800/80">
                <x-app-logo :sidebar="true" href="{{ route('dashboard') }}" wire:navigate />
                <flux:sidebar.collapse class="lg:hidden" />
            </flux:sidebar.header>

            <flux:sidebar.nav class="py-3">
            ...
            <flux:sidebar.nav class="border-t border-zinc-100/80 py-2 dark:border-zinc-800">
        ...
        <flux:header class="sticky top-0 z-20 border-b border-zinc-200/80 bg-white/90 px-4 backdrop-blur-md dark:border-zinc-800/80 dark:bg-zinc-950/90 sm:px-6 lg:px-8">
            <flux:sidebar.toggle class="lg:hidden" icon="bars-2" inset="left" />

            <div class="hidden min-w-0 flex-col lg:flex">
                <flux:heading size="lg" class="font-semibold tracking-tight text-zinc-900 dark:text-white">
                    {{ filled($title ?? null) ? __($title) : __('Dashboard') }}
                </flux:heading>
                <flux:text class="truncate text-xs font-medium text-zinc-500 dark:text-zinc-400">
            ...
            <flux:navbar class="me-2 gap-0.5 rtl:space-x-reverse">
                @canany(['settings.view', 'branding.view'])
                ...
                <flux:dropdown position="bottom" align="end">
                    <flux:tooltip :content="__('Theme')" position="bottom">
                        <flux:navbar.item icon="moon" href="#" :label="__('Theme')" />
                    </flux:tooltip>
                    <flux:menu>
                        <flux:menu.item icon="sun" :href="route('appearance.edit')" wire:navigate>{{ __('Light') }}</flux:menu.item>
                        <flux:menu.item icon="moon" :href="route('appearance.edit')" wire:navigate>{{ __('Dark') }}</flux:menu.item>
                        <flux:menu.item icon="computer-desktop" :href="route('appearance.edit')" wire:navigate>{{ __('System') }}</flux:menu.item>
                    </flux:menu>
                </flux:dropdown>

                <flux:dropdown position="bottom" align="end">
                    <flux:tooltip :content="__('Language')" position="bottom">
                        <flux:navbar.item icon="language" href="#" :label="__('Language')" />
                    </flux:tooltip>
                    <flux:menu>
            ...
            <flux:dropdown position="bottom" align="end">
                <flux:profile :initials="$user->initials()" icon-trailing="chevron-down" class="ms-1" />
                <flux:menu>
                    <div class="flex items-center gap-3 px-3 py-2.5">
                        <flux:avatar :name="$user->name" :initials="$user->initials()" class="ring-2 ring-zinc-200/80 dark:ring-zinc-700" />
        ...
        <flux:main class="bg-zinc-50/80 dark:bg-zinc-950">
