  ...
  <body class="min-h-screen bg-zinc-100 text-zinc-900 antialiased dark:bg-zinc-950 dark:text-zinc-100">
    <main class="flex min-h-screen items-center justify-center p-6">
      <section class="w-full max-w-lg rounded-lg border border-zinc-200 bg-white p-8 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div class="mx-auto flex size-16 items-center justify-center rounded-full bg-amber-100 text-2xl font-bold text-amber-700 dark:bg-amber-950 dark:text-amber-200">{{ $status }}</div>
        <h1 class="mt-6 text-2xl font-semibold">{{ __('Forbidden') }}</h1>
        <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-300">{{ __('You do not have permission to open this page.') }}</p>
