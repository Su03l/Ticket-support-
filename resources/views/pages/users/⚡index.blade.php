...
<div x-data="{ showInvite: false }" class="flex flex-col gap-6">
    <x-page-header
        :title="__('Users')"
        :description="__('Invite employees, assign departments, and manage account status.')"
    >
        <x-slot:actions>
            @can('create', \App\Models\UserInvitation::class)
                <flux:button x-on:click="showInvite = !showInvite" variant="primary" icon="user-plus">
                    {{ __('Invite user') }}
                </flux:button>
            @endcan
        </x-slot:actions>
    ...
    @can('create', \App\Models\UserInvitation::class)
        <div x-show="showInvite" x-collapse x-cloak>
            <x-section-card :heading="__('Invite user')" icon="envelope" variant="info">
                <x-slot:headerAction>
                ...
                <form wire:submit="invite">
                    ...
                    <div class="mt-5 flex items-center justify-end gap-3 border-t border-zinc-100/80 pt-4 dark:border-zinc-800">
                        ...
                        <flux:button variant="primary" type="submit" icon="paper-airplane">{{ __('Send invitation') }}</flux:button>
                    </div>
                </form>
            </x-section-card>
        </div>
                ...
                <div class="flex gap-2 lg:justify-end">
                    @can('suspend', $managedUser)
                        @if ($managedUser->status !== \App\Enums\UserStatus::Suspended)
                            <flux:button size="sm" variant="danger" wire:click="suspend({{ $managedUser->id }})">{{ __('Suspend') }}</flux:button>
                        @endif
                    @endcan
                    @can('restore', $managedUser)
                        @if ($managedUser->status !== \App\Enums\UserStatus::Active)
                            <flux:button size="sm" variant="primary" wire:click="restore({{ $managedUser->id }})">{{ __('Activate') }}</flux:button>
                        @endif
                    @endcan
                </div>
            </x-list-row>
