...
<div class="flex flex-col gap-6">
 ...
 @can('create', \Spatie\Permission\Models\Role::class)
  <x-section-card :heading="__('Create new role')" icon="shield-check">
 ...
 <div class="overflow-x-auto rounded-[2rem] border border-zinc-200/60 bg-white p-2 shadow-sm dark:border-zinc-800/60 dark:bg-zinc-900">
  <div class="flex min-w-max gap-2 px-1">
   @foreach ($roles as $role)
    <div wire:key="role-tab-{{ $role->id }}" class="flex items-center gap-1 rounded-2xl transition-all {{ $selectedRoleId === $role->id ? 'bg-zinc-900 text-white shadow-lg dark:bg-white dark:text-zinc-900' : 'bg-zinc-50 text-zinc-600 hover:bg-zinc-100 dark:bg-zinc-800/40 dark:text-zinc-400 dark:hover:bg-zinc-800' }}">
     <button type="button" wire:click="selectRole({{ $role->id }})" class="px-5 py-2 text-sm font-bold ">
      {{ $role->name }}
      <span class="ms-2 inline-flex items-center justify-center rounded-lg px-1.5 py-0.5 text-[10px] font-semibold {{ $selectedRoleId === $role->id ? 'bg-white/20 text-white dark:bg-black/10 dark:text-zinc-900' : 'bg-zinc-200/60 text-zinc-500 dark:bg-zinc-700 dark:text-zinc-500' }}">
       {{ $role->permissions_count ?? $role->permissions->count() }}
      </span>
     </button>
     @can('delete', $role)
      <button type="button" wire:click="deleteRole({{ $role->id }})" class="me-2 rounded-lg p-1.5 transition-colors hover:bg-red-500/20 hover:text-red-500"aria-label="{{ __('Delete role') }}">
 ...
 @if ($selectedRoleId)
  <x-section-card icon="key">
   ...
   <x-slot:headerAction>
    <flux:button variant="primary" size="sm" wire:click="savePermissions">{{ __('Save permissions') }}</flux:button>
   </x-slot:headerAction>
   ...
   <div class="flex flex-col gap-6">
    ...
    <div class="grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
     @foreach ($permissionGroups as $module => $permissions)
      <div wire:key="permission-group-{{ $module }}" class="rounded-2xl border border-zinc-100 bg-zinc-50/30 p-5 dark:border-zinc-800/60 dark:bg-zinc-800/20 transition-all hover:bg-white hover:shadow-md dark:hover:bg-zinc-800/40">
       <flux:heading size="sm" class="font-bold tracking-wider uppercase text-blue-600 dark:text-blue-400 border-b border-zinc-100 dark:border-zinc-800 pb-2 mb-4">{{ __(str_replace('_', ' ', $module)) }}</flux:heading>
    ...
    <div class="mt-4 flex justify-end border-t border-zinc-100/80 pt-6 dark:border-zinc-800">
     <flux:button variant="primary" wire:click="savePermissions" class="px-8">{{ __('Save changes') }}</flux:button>
    </div>
   </div>
  </x-section-card>
 @else
  <x-empty-state icon="shield-check":heading="__('Select a role')":description="__('Click on a role above to manage its permissions and system access.')" class="py-14"/>
 @endif
</div>